...
- **アドベンチャーモード**: ボスを倒してマップを進む「すごろく」形式の冒険モード。エリアごとに背景やテーマが変化し、トレーニングの達成感を視覚化します。
...
4. **ミッション**: 画面上部の「今日のミッション」を確認し、達成を目指します。
5. **装備**: 武器を手に入れたら「装備」ボタンから最強の武器を装備しましょう。
6. **センサーモード**: センサーモードを使う場合はスマホを太ももの前面に固定してオンにします。
7. **実績確認**: 「過去の実績」カードのタブを切り替えて、獲得したバッジや履歴を確認できます。
8. **称号設定**: 実績画面の「称号設定」から、獲得した称号を組み合わせてプロフィールに表示できます。
9. **シェア**: トレーニング完了画面の「シェア画像を作成」ボタンから、本日の戦績カードを保存してシェアできます。
