...
## ドキュメント同期レポート - 2026-01-30

### 検出された変更
- **クラスシステム (Class System)**: ユーザーが職業（クラス）を選択し、ステータス補正を得られる機能が実装されました。
  - 実装: `js/modules/class-manager.js`, `js/data/classes.json`
  - UI: ヘッダーパネルのクラスアイコンから変更可能。
...
### 注意が必要な項目
- **仕様書の場所**: クラスシステムの仕様書は `.sdd-archive/specs/class-system/` に存在し、現在の実装と整合していますが、現行の `.specify/` ディレクトリには移行されていません。
...
## ドキュメント同期レポート - 2026-02-01

### 検出された変更
- **ストリーク・ガーディアン (Streak Guardian)**: 連続記録の途切れ防止機能（残り時間表示、ステータス通知）が実装されています。
  - 実装: `js/modules/streak-guardian.js`
  - 仕様書: `.specify/specs/003-streak-guardian/spec.md`

### 更新したドキュメント
- **README.md**:
  - 「主な機能」に「ストリーク・ガーディアン」を追加しました。
- **.specify/specs/spec-001-weekly-challenge/tasks.md**:
  - 前回レポートで欠落が指摘されていたタスクリストファイルを作成し、実装済みステータスを反映しました。
...
### 検出された変更
- **音声コマンド操作 (Voice Command Control)**: ハンズフリーでトレーニング操作を行うための音声認識機能が実装されました。
  - 実装: `js/modules/voice-control.js`
  - 仕様書: `.specify/specs/004-voice-command/spec.md`
  - 対応コマンド: 「スタート」「ストップ（一時停止）」「リセット」

### 更新したドキュメント
- **タスクリストの同期**: 以下の機能について、実装が完了していることを確認し、タスクリストを全て完了状態 (`[x]`) に更新しました。
  - `.specify/specs/001-bestiary/tasks.md` (モンスター図鑑)
  - `.specify/specs/003-streak-guardian/tasks.md` (ストリーク・ガーディアン)
  - `.specify/specs/004-voice-command/tasks.md` (音声コマンド)

### 注意が必要な項目
- 特になし。`README.md` は既に最新機能（音声コマンド含む）を反映済みであることを確認しました。
...
## ドキュメント同期レポート - 2026-02-03

### 検出された変更
- **クラス・マスタリー（熟練度）システム**: `feat/class-mastery` ブランチで実装された、クラスの熟練度システムがマージされました。
  - 実装: `js/modules/class-manager.js` の更新（EXP獲得、レベルアップロジック、ボーナス計算）
  - UI: クラス選択画面でのレベル表示、進捗バーの追加。
  - 仕様書: `.specify/specs/spec-005-class-mastery/`

### 更新したドキュメント
- **README.md**:
  - 「クラスシステム」の説明に、熟練度によるレベルアップとボーナス強化についての記述を追加しました。
- **.specify/specs/spec-005-class-mastery/tasks.md**:
  - 実装完了に伴い、全てのタスクを完了済み (`[x]`) に更新しました。
...
### 検出された変更
- `README.md` とコードベースの実装状況はおおむね同期されていることを確認しました。
- 一部の仕様書フォルダ (`.specify/specs/`) において、実装済み機能のタスクリスト (`tasks.md`) が未完了のままか、ファイル自体が存在しない状態が検出されました。

### 更新したドキュメント
- `.specify/specs/001-commitment-feature/tasks.md`: 全タスクを実装完了 (`[x]`) に更新しました。
- `.specify/specs/share-card/tasks.md`: 新規作成し、実装済みのタスクを記録しました。
- `.specify/specs/002-tension-boost/tasks.md`: 新規作成し、実装済みのタスクを記録しました。

### 注意が必要な項目
- 特になし。

### 推奨事項
- 今後新機能を実装する際は、`spec.md` と同時に `tasks.md` を作成し、進捗に合わせて更新することを推奨します。
...
### 検出された変更
- **クラス・アクティブスキル**: クラスシステムにアクティブスキル（セッション中1回使用可能）が実装されていることを確認しました。
  - 実装: `js/modules/skill-manager.js`, `js/data/classes.json`
  - テスト: `tests/verification/spec-006-skills.spec.js`

### 更新したドキュメント
- **.specify/specs/spec-006-class-skills/tasks.md**:
  - 実装済みにもかかわらずタスクが全て未完了(`[ ]`)であったため、全タスクを完了(`[x]`)に更新しました。
- **README.md**:
  - 「クラスシステム」の項目を更新し、アクティブスキルの存在とその効果（攻撃力1.5倍、クイズ自動正解など）について明記しました。

### 注意が必要な項目
- 特になし。

### 推奨事項
- 実装完了時は速やかに `tasks.md` を更新することを推奨します。

---

## ドキュメント同期レポート - 2026-02-06

### 検出された変更
- **アドベンチャーモード「ルート選択」機能**: エリア進行時やクリア時に、難易度と報酬が異なる3つのルート（王道、修羅の道、裏道）を選択する分岐イベントが実装されました。
  - 実装: `js/modules/adventure-system.js` (Routeロジック), `js/modules/boss-battle.js` (HP補正反映), `index.html` (モーダル)
  - テスト: `tests/spec-007-route-selection.spec.js`
- **プリセットマネージャー**: ワークアウト設定の保存・読み込み機能の実装を確認しました。
  - 実装: `js/modules/preset-manager.js`

### 更新したドキュメント
- **.specify/specs/spec-007-route-selection/tasks.md**:
  - 実装済み機能に対応するタスクを全て完了状態 (`[x]`) に更新しました。
- **README.md**:
  - 「アドベンチャーモード」の説明に、ルート選択（分岐イベント）に関する記述を追加しました。

### 注意が必要な項目
- 特になし。テスト (`tests/spec-007-route-selection.spec.js`) もパスしており、実装とドキュメントの整合性は取れています。
